...
const getRenameColumn = renameData => {
	const script = `${alterTablePrefix(renameData.tableName, renameData.keyspaceName)} ${renameColumnStatement(renameData.columnData)};`;
	return [
		{
			added: false,
			deleted: false,
...
const getIsColumnInIndex = (item, columnName, data) => {
	...
	const secIndexes = _.get(item, 'role.SecIndxs', [])
		.map(index => getDataColumnIndex(dataSources, {}, index, 'SecIndxKey'))
...
const getAlterTableScript = (child, udtMap, data) => {
	...
	if (objectContainsProp(child, 'containers')) {
		alterScript = mergeArrays(
			alterScript,
			getScript(child.containers, udtMap, data, 'containers', undefined, data.scriptOptions),
		);
	}
...
const sortScript = scriptData => {
	...
	const sortedScripts = orderForScripts.reduce(
		(script, [key, prop]) => {
